...
def calc_position(width, height, center_x, center_y):
    xmin = x-width
    ymin = y-height
    xmax = x+1+width
    ymax = y+1+height
...
for i in range(n):
    ...
    ans_list[i] = calc_position(width, width, x, y)
        ...
        while count <= count_limit:
            if overlap(ans_list[i], ans_list[j]):
                ...
                if random.random() < 0.5:
                    ...
                    width = width * 9 // 10
                    width_list[i] = width
                    ans_list[i] = calc_position(width, width, xs[i], ys[i]).copy()
                    ...
                    width = width * 9 // 10
                    width_list[j] = width
                    ans_list[j] = calc_position(width, width, xs[j], ys[j]).copy()
...
print(xs)
print(ys)
print([id(ele) for ele in ans_list])